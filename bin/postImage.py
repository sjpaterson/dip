...
for subchan in subchans:
    ...
    pb_warp = os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp.fits')
    ...
    if not os.path.exists(os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_comp.fits')):
        ...
        subprocess.run('lookup_beam.py ' + pb + ' ' + obsid + '_deep-' + subchan + '-image-pb-' + '-c "' + chans[chanStart] + '-' + chans[chanEnd] + '"', shell=True, check=True, cwd=obsdir)
        ...
        subprocess.run('BANE --cores 48 --compress --noclobber "' + pb + '"', shell=True, check=True, cwd=obsdir)
        subprocess.run('aegean --cores 1 --autoload --table="' + pb + '" "' + pb + '"', shell=True, check=True, cwd=obsdir)
        
        
    # Check that a sufficient number of sources were detected.
    catHdu = fits.open(os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_comp.fits'))
    cat = catHdu[1].data
    nsrc = len(cat)
    catHdu.close()
    ...
    if not os.path.exists(os.path.join(obsdir, obsid + '_' + subchan + '_complete_sources_xm.fits')):
        subprocess.run('fits_warp.py --incat "' + obsid + '_deep-' + subchan + '-image-pb_comp.fits" --refcat "' + POS_MODEL_CATALOGUE + '" --xm "' + obsid + '_' + subchan + '_complete_sources_xm.fits" --plot --ra1 ra --dec1 dec --ra2 RAJ2000 --dec2 DEJ2000 --infits "' + pb + '"', shell=True, check=True, cwd=obsdir)
    
    if not os.path.exists(pb_warp):
        subprocess.run('fits_warp.py --incat "' + obsid + '_deep-' + subchan + '-image-pb_comp.fits" --refcat "' + POS_MODEL_CATALOGUE + '" --corrected "' + obsid + '_deep-' + subchan + '-image-pb_comp_warp-corrected.fits" --xm "' + obsid + '_' + subchan + '_fits_warp_xm.fits" --suffix warp --infits "' + pb + '" --ra1 ra --dec1 dec --ra2 RAJ2000 --dec2 DEJ2000 --plot --nsrcs 750 --vm 10 --progress --cores 24 --signal peak_flux_1 --enforce-min-srcs 100', shell=True, check=True, cwd=obsdir)

    # Flux_warp dependency, match the image catalogue to the model table.
    if not os.path.exists(os.path.join(obsdir, obsid + '_' + subchan + '_xm.fits')):
        subprocess.run('match_catalogues "' + obsid + '_deep-' + subchan + '-image-pb_comp_warp-corrected.fits" "' + FLUX_MODEL_CATALOGUE + '" --separation "' + separation + '" --exclusion_zone "' + exclusion + '" --outname "' + obsid + '_' + subchan + '_xm.fits" --threshold 0.5 --nmax 1000 --coords ' + str(metadata['RA']) + ' ' + str(metadata['DEC']) + ' --radius "' + str(radius) + '" --ra2 "RAJ2000" --dec2 "DEJ2000" --ra1 "ra" --dec1 "dec" -F "int_flux" --eflux "err_int_flux" --localrms "local_rms"', shell=True, check=True, cwd=obsdir)

    if not os.path.exists(os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_scaled_cf_output.txt')):
        subprocess.run('flux_warp "' + obsid + '_' + subchan + '_xm.fits" "' + obsid + '_deep-' + subchan + '-image-pb_warp.fits" --mode mean --freq "' + freqq + '" --threshold 0.5 --nmax 400 --flux_key "flux" --smooth 5.0 --ignore_magellanic --localrms_key "local_rms" --add-to-header --ra_key "RAJ2000" --dec_key "DEJ2000" --index "alpha" --curvature "beta" --ref_flux_key "S_200" --ref_freq 200.0 --alpha -0.77 --plot --cmap "gnuplot2" --update-bscale --order 2 --ext png --nolatex', shell=True, check=True, cwd=obsdir)
    ...
    subprocess.run('mv "' + os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_rms.fits') + '" "' + os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_rms.fits') + '"', shell=True)
    subprocess.run('mv "' + os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_bkg.fits') + '" "' + os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_bkg.fits') + '"', shell=True)
    fits.setval(os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_rms.fits'), 'BSCALE', value=factor)
    fits.setval(os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_bkg.fits'), 'BSCALE', value=factor)


    # Rerun the source finding.
    if not os.path.exists(os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_comp.fits')):
        subprocess.run('aegean --cores 1 --autoload --table="' + pb_warp + '" "' + pb_warp + '"', shell=True, check=True, cwd=obsdir)

    # Generate a weight map for mosaicking.
    if not os.path.exists(os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_weight.fits')):
        subprocess.run('lookup_beam.py "' + obsid + '" "_deep-' + subchan + '-image-pb_warp.fits" "' + obsid + '_deep-' + subchan + '-image-pb_warp-" -c "' + chans[chanStart] + '-' + chans[chanEnd] + '"', shell=True, check=True, cwd=obsdir)
        gwm.genWeightMap(os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp-XX-beam.fits'), os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp-YY-beam.fits'), os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_rms.fits'), os.path.join(obsdir, obsid + '_deep-' + subchan + '-image-pb_warp_weight.fits'))
